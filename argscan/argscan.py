...
def main():
    ...
    try:
        ...
        yaml_data = yaml.dump(args_info, allow_unicode=True, sort_keys=False, default_style='"')  # デフォルトスタイルをダブルクォートに設定
        
        if args.output:
            with open(args.output, 'w', encoding='utf-8') as f:
                f.write(yaml_data)
        else:
            print(yaml_data, end='')  # 余分な改行を削除
